...
export default function Grocery() {
  ...
  const categories = [
    'Rice & Atta', 'Pulses', 'Oil & Ghee', 'Dairy', 
  ...
  return (
    <div className="flex flex-col pb-24 max-w-7xl mx-auto w-full animate-fade-in bg-gray-50 min-h-screen">
      ...
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md shadow-sm border-b border-line px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="font-display font-extrabold text-lg text-main">Grocery</h1>
      ...
      <div className="px-4 py-6">
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-3xl p-6 text-white shadow-md relative overflow-hidden">
          <div className="relative z-10 max-w-[200px]">
            <h2 className="font-display font-black text-2xl leading-tight mb-2">Fresh groceries delivered to your door</h2>
            <button className="bg-white text-green-700 font-bold text-xs px-4 py-2 rounded-xl mt-2">Shop Now</button>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {categories.map((cat, idx) => (
            <div key={idx} className="flex flex-col items-center gap-2 cursor-pointer group">
              <div className="w-16 h-16 rounded-2xl bg-white shadow-sm border border-line flex items-center justify-center group-hover:border-green-500 transition-colors">
      ...
      <div className="px-4">
        <h3 className="font-display font-bold text-lg text-main mb-4">Popular Items</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white rounded-2xl p-3 shadow-sm border border-line flex flex-col gap-2">
              ...
              <h4 className="font-bold text-sm text-main line-clamp-1">Sample Grocery Product {i}</h4>
              <div className="flex items-center justify-between mt-auto">
                <span className="font-black text-sm text-main">₹99</span>
                <button className="bg-green-50 text-green-700 font-bold text-xs px-3 py-1.5 rounded-lg border border-green-200">Add</button>
              </div>
            </div>
          ))}
        </div>
